			...
			<div class="row">
				<div class="col-lg-10 col-xl-7 m-lr-auto m-b-50">
					<div class="m-l-25 m-r--38 m-lr-0-xl">
						<button onclick="history.go(-1); event.preventDefault();"
						...
						<div class="wrap-table-shopping-cart">
							...
							<div class="row row-cart-intestazione">
								<div class="col-md-2 cart-intestazione">Prodotto</div>
								<div class="col-md-4 cart-intestazione"></div>
								<div class="col-md-2 cart-intestazione-prezzo">Prezzo</div>
								<div class="col-md-2 cart-intestazione">Quantit&agrave;</div>
								<div class="col-md-2 cart-intestazione-prezzo">Totale</div>
							</div>

							<s:iterator value="c.dettagli" status="stat">
								<div class="row row-cart-dato">
									<div class="col-md-2 col-sm-2 col-3 cart-dato">
										<div class="how-itemcart1" onclick="removprod2('<s:property value="%{codice_prodotto}"/>','<s:property value="%{descrizione}"/>');">
										<img width="120px"
											src="https://www.gdsmarket.it/immagini/<s:property value="%{prodotto.%{codice_prodotto}}" />/<s:property value="%{prodotto.nome_immagine}" />.jpg" onclick="removprod2('<s:property value="%{codice_prodotto}"/>','<s:property value="%{descrizione}"/>');"
											onerror="this.src='images/dish.jpg'"
											alt="IMG">
										</div>
									</div>

									<div class="col-md-4 col-sm-7 col-9 cart-dato">
										<a href="product.html?codice_prodotto=<s:property value='%{codice_prodotto}'/>" class="cl4 hov-cl1 trans-04 js-name-b2 p-b-6">
										<span id="desc_<s:property value='%{codice_prodotto}'/>"><s:property escape="false" value='%{descrizione}'/></span>
										</a>
									</div>

									<div class="col-md-2 col-sm-3 col-4 cart-dato-prezzo">
										<s:if test="prodotto.unita_misura=='cf'">
											&euro; <span id="pu_<s:property value='%{codice_prodotto}'/>"> <s:property value="getText('{0,number,#,##0.00}',{prezzo})"/></span>
										</s:if>
										<s:else>
											&euro; <span id="pu_<s:property value='%{codice_prodotto}'/>"> <s:property value="getText('{0,number,#,##0.00}',{prezzo/10})"/> / etto</span>
										</s:else>

									</div>
									<div class="col-md-2 col-sm-8 col-4 cart-dato">
										...
										<s:if test="prodotto.unita_misura=='cf'">
											...
											<div class="wrap-num-product flex-w m-l-auto m-r-0">
												...
												<input class="mtext-104 cl3 txt-center num-product" type="number" name="num-product1" data-id="<s:property value='%{codice_prodotto}'/>" value="<s:number name='%{quantita}' type='integer'/>">
...
<!-- 													type="number" -->
<!-- 													name="num-product1" -->
<%-- 													data-id="<s:property value='%{codice_prodotto}'/>" --%>
<%-- 													value="<s:property value='%{quantita * 1000}'/>"> --%>
										</s:else>


									</div>
									<div class="col-md-2 col-sm-4 col-4 cart-dato-prezzo-tot">&euro; <span id="pt_<s:property value='%{codice_prodotto}'/>"> <s:property value="getText('{0,number,#,##0.00}',{prezzo * quantita})"/></span></div>
								</div>
							</s:iterator>
						</div>

						<div class="flex-w flex-sb-m bor15 p-t-18 p-b-15 p-lr-15-sm">
...
<%-- 							</s:if> --%>
				...
				<div class="col-sm-10 col-lg-7 col-xl-5 m-lr-auto m-b-50">
					<div class="bor10 p-lr-40 p-t-30 p-b-40 m-l-63 m-r-40 m-lr-0-xl p-lr-15-sm">
						...
						<s:if test="%{c.totcart > 0}">
							<a href="delivery.html" class="flex-c-m cl0 size-116 bg3 bor14 hov-btn3 p-lr-15 trans-04 pointer">
								Procedi
							</a>
						</s:if>
						...
						<div class="bg-warning" style="font-size:0.9em!important" >
							...
							<strong><i class="fa fa-phone"></i><a href="tel:+3903811976703"> 0381 1976703</a></strong>.
						</div>
...
// 					$(this).parent().closest("#pt_" + id).text(Number(prezzototale).toFixed(2));
					...
					$.ajax(
							{
								url : "updqta.html?codice_prodotto=" + id + "&quantita=" + (numProduct - 1) + "&descrizione=" + descrizione,
								context : document.body
							}).done(function() {
...
// 							$(this).parent().closest("#pt_" + id).text(Number(prezzototale).toFixed(2));
							...
							if ((numProduct + 1) == 0) {
								...
								if (r == true) {
									$
											.ajax(
													{
														url : "remfromcart.html?codice_prodotto="
																+ codice_prodotto + "&descrizione=" + descrizione,
														context : document.body
													})
											.done(
													function() {
		...
		function removprod2(codice_prodotto, descrizione) {
			var r = confirm("Vuoi rimuovere questo prodotto dal carrello?");
